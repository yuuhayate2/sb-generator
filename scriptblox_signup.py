...
def user_key_hash(k):     return hashlib.sha256(k.encode()).hexdigest()[:20]
def user_webhook_path(k): return USER_DATA_DIR / f"{user_key_hash(k)}.webhook"
def user_proxies_path(k): return USER_DATA_DIR / f"{user_key_hash(k)}.proxies"
def user_accounts_path(k):return USER_DATA_DIR / f"{user_key_hash(k)}.accounts.jsonl"

def load_user_webhook(k):
    p = user_webhook_path(k)
    return p.read_text().strip() if p.exists() else ""

def save_user_webhook(k, wh): user_webhook_path(k).write_text(wh or "")

def load_user_proxies(k):
    p = user_proxies_path(k)
    if p.exists():
        proxies = [l.strip() for l in p.read_text().splitlines() if l.strip() and not l.startswith("#")]
        if proxies: return proxies
    g = Path(__file__).parent / "proxies.txt"
    if g.exists():
        try:
            proxies = [l.strip() for l in g.read_text().splitlines() if l.strip() and not l.startswith("#")]
            if proxies: return proxies
        except: pass
    return []

def save_user_proxies(k, lines): user_proxies_path(k).write_text("\n".join(lines))

def create_session(license_key, license_record, ip, ua_hash_val):
    token = secrets.token_urlsafe(32)
    with sessions_lock:
        sessions[token] = {
            "license_key": license_key, "license_record": license_record,
            "webhook": load_user_webhook(license_key), "proxies": load_user_proxies(license_key),
            "state": fresh_state(), "ip": ip, "ua_hash": ua_hash_val,
            "created_at": time.time(), "last_seen": time.time(), "file_lock": threading.Lock(),
        }
    return token
    ...
    while True:
        ...
        try:
            with sessions_lock:
                ...
                dead = [sid for sid, t in sid_to_token.items() if t not in sessions]
                for sid in dead: sid_to_token.pop(sid, None)
        except: pass
...
def require_http_session():
    ...
    token = (request.headers.get("X-Session-Token") or (body or {}).get("session_token", "")).strip()
...
def get_client_ua(): return (request.headers.get("User-Agent") or "")[:500]
def ua_hash(ua): return hashlib.sha256((ua or "").encode()).hexdigest()[:20]
...
DATACENTER_PREFIXES = (
    ...
    "172.70.","188.114.","34.","35.192.","35.193.","35.194.","35.195.","35.196.",
    "35.197.","35.198.","35.199.","35.200.","35.201.","35.202.","35.203.","35.204.",
    "52.","54.","18.","3.","157.90.","159.69.","95.216.","116.202.","168.119.",
    "46.101.","159.89.","165.227.","134.209.","167.99.","138.197.","138.68.",
    "45.76.","45.77.","108.61.","149.28.","155.138.","207.148.","66.42.","45.32.",
    "185.244.","185.159.","185.232.","185.220.","89.187.","193.32.","185.65.","185.107.",
)
    ...
    with counter_lock:
        for attempt in range(3):
            try:
                ...
                if limit < 9999 and used >= limit: return (used, False)
                new_val = used + 1
                r = requests.patch(f"{SUPABASE_URL}/rest/v1/licenses",
                                   headers={**supa_hdrs(), "Prefer": "return=representation"},
                                   params={"license_key": f"eq.{key}", "accounts_used": f"eq.{used}"},
...
def sb_verify_account(code, token_value, proxy_r=None, visitor_id=None):
    try:
        ...
        r = requests.post(SB_VERIFY, json={"vCode": int(code)}, headers=hdrs,
                          cookies={"token": token_value, "visitor": visitor_id},
...
def sb_login(email, password, proxy_r=None):
    try:
        r = requests.post(SB_LOGIN, json={"login": email, "password": password},
...
def _rand_gpi():
    ...
    return f"UID={uid}:T={ts}:RT={int(time.time())}:S=ALNI_{sfx}"
def _rand_eoi():
    uid = "".join(random.choices("0123456789abcdef", k=16))
    ts = int(time.time()) - random.randint(0, 86400*30)
    sfx = "".join(random.choices(string.ascii_letters + string.digits + "_-", k=20))
    return f"ID={uid}:T={ts}:RT={int(time.time())}:S=AA-Afj{sfx}"
def _rand_gads():
    uid = "".join(random.choices("0123456789abcdef", k=16))
    ts = int(time.time()) - random.randint(0, 86400*30)
    sfx = "".join(random.choices(string.ascii_letters + string.digits + "_-", k=20))
    return f"ID={uid}:T={ts}:RT={int(time.time())}:S=ALNI_{sfx}"
def _rand_fcnec():
    inner = "".join(random.choices(string.ascii_letters + string.digits + "_-", k=80))
    return quote(f'[["AKsRol_{inner}=="]]')
def _rand_ga6():
    return f"GS2.1.s{int(time.time()) - random.randint(0, 3600)}$o5$g1$t{int(time.time())}$j59$l0$h0"
def _rand_visitor():
...
def fabricate_full_cookies(token_value, username, verified=True, visitor_id=None):
    ...
    return [
        {"domain":"scriptblox.com","expirationDate":now+3600,"hostOnly":True,"httpOnly":False,"name":"__scriptblox_ua_","path":"/","sameSite":"no_restriction","secure":True,"session":False,"storeId":None,"value":_rand_ua_cookie()},
        {"domain":"scriptblox.com","hostOnly":True,"httpOnly":False,"name":"visitor","path":"/","sameSite":None,"secure":False,"session":True,"storeId":None,"value":visitor},
        {"domain":"scriptblox.com","expirationDate":creation+86400*365,"hostOnly":True,"httpOnly":False,"name":"i18n_redirected","path":"/","sameSite":"lax","secure":False,"session":False,"storeId":None,"value":"en"},
        {"domain":"scriptblox.com","expirationDate":now+86400*30,"hostOnly":True,"httpOnly":True,"name":"token","path":"/","sameSite":"strict","secure":True,"session":False,"storeId":None,"value":token_value},
        {"domain":".scriptblox.com","expirationDate":creation+86400*400,"hostOnly":False,"httpOnly":False,"name":"_ga","path":"/","sameSite":None,"secure":False,"session":False,"storeId":None,"value":_rand_ga_id()},
        {"domain":".scriptblox.com","expirationDate":now+86400,"hostOnly":False,"httpOnly":False,"name":"_gid","path":"/","sameSite":None,"secure":False,"session":False,"storeId":None,"value":_rand_gid()},
        {"domain":".scriptblox.com","expirationDate":creation+86400*400,"hostOnly":False,"httpOnly":False,"name":"_ga_6BWTBXZCLM","path":"/","sameSite":None,"secure":False,"session":False,"storeId":None,"value":_rand_ga6()},
        {"domain":".scriptblox.com","expirationDate":creation+86400*390,"hostOnly":False,"httpOnly":False,"name":"__gpi","path":"/","sameSite":"no_restriction","secure":True,"session":False,"storeId":None,"value":_rand_gpi()},
        {"domain":".scriptblox.com","expirationDate":creation+86400*180,"hostOnly":False,"httpOnly":False,"name":"__eoi","path":"/","sameSite":"no_restriction","secure":True,"session":False,"storeId":None,"value":_rand_eoi()},
        {"domain":".scriptblox.com","expirationDate":creation+86400*390,"hostOnly":False,"httpOnly":False,"name":"__gads","path":"/","sameSite":"no_restriction","secure":True,"session":False,"storeId":None,"value":_rand_gads()},
        {"domain":".scriptblox.com","expirationDate":now+86400*390,"hostOnly":False,"httpOnly":False,"name":"FCNEC","path":"/","sameSite":None,"secure":False,"session":False,"storeId":None,"value":_rand_fcnec()},
    ]
...
def send_webhook(webhook_url, username, password, email, cookies_url=None, cookies_json=None, verify_status="unverified"):
    ...
    try:
        ...
        fields = [
            {"name": "👤 Username", "value": f"```\n{username}\n```", "inline": True},
            {"name": f"{status_emoji} Status", "value": f"```\n{'Verified' if is_verified else 'Unverified'}\n```", "inline": True},
            {"name": "📧 Email", "value": f"```\n{mask_email(email)}\n```", "inline": False},
        ]
        if cookies_url:
            fields.append({"name": "🍪 Session Cookies", "value": f"> **Ready for import**\n> Import via **Cookie-Editor** extension\n\n[📥 **Download cookies.json**]({cookies_url})", "inline": False})
        else:
            fields.append({"name": "🍪 Session Cookies", "value": "> **Session cookies included**\n> Attached as file below", "inline": False})
        embed = {
            "author": {"name": "KUNI · SB GENERATOR", "icon_url": "https://cdn.discordapp.com/attachments/1485145708666552342/1495745856362516591/content.png"},
            "title": "New Account Generated",
            "description": "A fresh ScriptBlox account is generated.\n───────────────────────────",
            "color": color, "fields": fields,
            "thumbnail": {"url": "https://cdn.discordapp.com/attachments/1485145708666552342/1495745856362516591/content.png"},
            "footer": {"text": f"Kuni Tool • {datetime.now(timezone.utc).strftime('%b %d, %Y %H:%M UTC')}", "icon_url": "https://cdn.discordapp.com/attachments/1485145708666552342/1495745856362516591/content.png"},
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
        payload = {"username": "Kuni SB Gen", "avatar_url": "https://cdn.discordapp.com/attachments/1485145708666552342/1495745856362516591/content.png", "embeds": [embed]}
...
def test_webhook(url):
    try:
        r = requests.post(url, json={
            ...
            "embeds": [{"author": {"name": "KUNI · SB GENERATOR"}, "title": "✅ Webhook Connected", "description": "Your webhook is now linked to **Kuni SB Generator**.\nGenerated accounts and session cookies will be delivered right here.\n───────────────────────────", "color": 0x00D4FF, "fields": [{"name": "✅ Status", "value": "```\nOnline\n```", "inline": True}, {"name": "🔗 Endpoint", "value": "```\nscriptblox.com\n```", "inline": True}, {"name": "📦 Delivery", "value": "```\nAccount + Cookies\n```", "inline": True}], "footer": {"text": f"Kuni Tool • {datetime.now(timezone.utc).strftime('%b %d, %Y %H:%M UTC')}"}, "timestamp": datetime.now(timezone.utc).isoformat()}]
...
def proxy_to_requests(proxy):
    ...
    if isinstance(proxy, str):
        p = proxy.strip()
        if p.startswith("http://") or p.startswith("https://"): return {"http": p, "https": p}
        parts = p.split(":")
        if len(parts) == 2: return {"http": f"http://{p}", "https": f"http://{p}"}
        if len(parts) == 4:
...
def create_account(sess_token, slot):
    ...
    try:
        ...
        }, headers=signup_hdrs, cookies={"visitor": visitor_id}, proxies=proxy_r, timeout=30, verify=False)
        resp = r.json() if r.content else {}
        print(f"[signup #{slot}] status={r.status_code} keys={list(resp.keys()) if isinstance(resp,dict) else type(resp)} msg={str(resp.get('message',''))[:150]}")
        signup_token = resp.get("token") or resp.get("accessToken") or ""
        if not signup_token and isinstance(resp.get("data"), dict): signup_token = resp["data"].get("token", "")
    ...
    if r.status_code >= 400 or resp.get("error") or (isinstance(resp.get("statusCode"), int) and resp["statusCode"] >= 400):
        state["failed"] += 1
        log_emit(sess_token, f"[#{slot}] x signup failed ({r.status_code}): {resp.get('message') or resp.get('error') or ''}", "err")
        return
    ...
    if not signup_token:
        for c in r.cookies:
            if c.name == "token" and c.value: signup_token = c.value; break
    if not signup_token:
        log_emit(sess_token, f"[#{slot}] no token in signup - trying login...", "dim")
        login_tok, _ = sb_login(email_addr, password, proxy_r)
        if login_tok:
        ...
        if ok:
            ...
            try:
                ...
                requests.get(f"{SB_HOME}?showWelcome=true",
                             ...
                             headers=home_hdrs, proxies=proxy_r, timeout=15, verify=False)
            except: pass
        else:
    ...
    webhook_ok = send_webhook(webhook_url, username, password, email_addr, cookies_url, cookies_json_str, verify_status)
    if not webhook_ok:
    ...
    account = {
        ...
        "verify_status": verify_status, "created_at": datetime.now(timezone.utc).isoformat(),
    }
    with sess["file_lock"]:
    ...
    try:
        ...
        if exp:
            try:
                exp_dt = datetime.fromisoformat(exp.replace("Z","+00:00"))
                if exp_dt.tzinfo is None: exp_dt = exp_dt.replace(tzinfo=timezone.utc)
                if exp_dt < datetime.now(timezone.utc): return jsonify({"valid": False, "error": "expired"})
            except: pass
...
def on_connect(auth):
    ...
    if not sess: emit("auth_failed"); return False
...
def on_start(data):
    sess = socket_session()
    if not sess: emit("auth_failed"); return
    if sess["state"]["running"]: return
    if not sess["webhook"]:
        emit("start_blocked", {"reason": "webhook_required", "message": "Set a Discord webhook first. Accounts cannot be delivered without it."})
    ...
    concurrent = max(1, min(int(data.get("concurrent", 10)), 50))
    if limit < 9999:
...
<style>
  *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
  :root {
    --bg: #080b0f; --surface: rgba(14,19,24,0.90); --surface2: rgba(20,28,36,0.95);
  ...
  html, body { height: 100%; background: linear-gradient(rgba(8,11,15,0.80), rgba(8,11,15,0.92)), url('https://i.imgur.com/V7sJePT.png') center/cover no-repeat fixed; color: var(--text); font-family: var(--mono); }
  body::after { content: ''; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: repeating-linear-gradient(transparent 0px, transparent 3px, rgba(0,212,255,0.03) 3px, rgba(0,212,255,0.03) 6px); pointer-events: none; z-index: 1; animation: scanline 12s linear infinite; }
  @keyframes scanline { 0% { background-position: 0 0; } 100% { background-position: 0 300px; } }
  .lic-wrap, .app { position: relative; z-index: 2; }
  .lic-card, .config-card, .stat, .limit-bar-wrap, .log-wrap, .tutorial-card { background: var(--surface); border: 1px solid rgba(0,212,255,0.25); box-shadow: 0 0 20px rgba(0,212,255,0.10), inset 0 1px 0 rgba(255,255,255,0.04); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); }
  .panel-wrap, .webhook-wrap { background: rgba(8,11,15,0.80); border: 1px solid rgba(0,212,255,0.20); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); }
...
<div id="app"></div>
<script>
let licenseInfo=null,sessionToken=null,socket=null,running=false,webhookSet=false,cfgOpen=true,tutOpen=false;
const ERR_MAP={not_found:'invalid license key - contact Kuni',disabled:'this license has been disabled',expired:'license has expired - contact Kuni',hwid_mismatch:'this key is bound to another machine/browser',limit_reached:'account limit reached - upgrade your plan',rate_limited:'too many attempts - wait 1 minute',server_error:'server error - try again later'};
function getLsToken(){let t=localStorage.getItem('_kuni_lst');if(!t){t=[...crypto.getRandomValues(new Uint8Array(16))].map(b=>b.toString(16).padStart(2,'0')).join('');localStorage.setItem('_kuni_lst',t);}return t;}
async function getCanvasFp(){try{const c=document.createElement('canvas'),ctx=c.getContext('2d');ctx.textBaseline='top';ctx.font='14px Arial';ctx.fillStyle='#f60';ctx.fillRect(125,1,62,20);ctx.fillStyle='#069';ctx.fillText('kuni-fp',2,15);ctx.fillStyle='rgba(102,204,0,0.7)';ctx.fillText('kuni-fp',4,17);return c.toDataURL().slice(-64);}catch{return '';}}
async function getAudioFp(){try{const AC=window.OfflineAudioContext||window.webkitOfflineAudioContext;if(!AC)return '';const ctx=new AC(1,5000,44100),osc=ctx.createOscillator(),comp=ctx.createDynamicsCompressor();osc.type='triangle';osc.frequency.value=10000;comp.threshold.value=-50;comp.knee.value=40;comp.ratio.value=12;comp.attack.value=0;comp.release.value=0.25;osc.connect(comp);comp.connect(ctx.destination);osc.start(0);const buf=await ctx.startRendering(),ch=buf.getChannelData(0);let sum=0;for(let i=4500;i<5000;i++)sum+=Math.abs(ch[i]);return sum.toString(36).slice(-20);}catch{return '';}}
function getWebGLFp(){try{const c=document.createElement('canvas'),gl=c.getContext('webgl')||c.getContext('experimental-webgl');if(!gl)return '';const ext=gl.getExtension('WEBGL_debug_renderer_info');const r=ext?gl.getParameter(ext.UNMASKED_RENDERER_WEBGL):(gl.getParameter(gl.RENDERER)||'');const v=ext?gl.getParameter(ext.UNMASKED_VENDOR_WEBGL):(gl.getParameter(gl.VENDOR)||'');return (r+'|'+v).slice(0,100);}catch{return '';}}
function getFontFp(){try{const fonts=['Arial','Courier New','Georgia','Times New Roman','Verdana','Trebuchet MS','Comic Sans MS','Impact','Tahoma'],test=document.createElement('span');test.style.position='absolute';test.style.visibility='hidden';test.style.fontSize='72px';test.textContent='mmmmmmmmmmlli';document.body.appendChild(test);const baseline={};['monospace','serif','sans-serif'].forEach(b=>{test.style.fontFamily=b;baseline[b]={w:test.offsetWidth,h:test.offsetHeight};});const detected=fonts.filter(f=>['monospace','serif','sans-serif'].some(b=>{test.style.fontFamily=`'${f}',${b}`;return test.offsetWidth!==baseline[b].w||test.offsetHeight!==baseline[b].h;}));document.body.removeChild(test);return detected.join(',');}catch{return '';}}
...
async function getDeviceFingerprint(){try{const raw=[navigator.userAgent,navigator.language,screen.width+'x'+screen.height,screen.colorDepth,new Date().getTimezoneOffset(),navigator.hardwareConcurrency||0,navigator.platform,navigator.maxTouchPoints||0,await getCanvasFp()].join('|');return await sha256Hex(raw);}catch{let id=localStorage.getItem('_did');if(!id){id=Math.random().toString(36).slice(2)+Math.random().toString(36).slice(2);localStorage.setItem('_did',id);}return id;}}
async function getExtraFp(){return await sha256Hex(await getAudioFp()+'|'+getWebGLFp()+'|'+getFontFp());}
async function authFetch(url,opts){opts=opts||{};const headers=Object.assign({'Content-Type':'application/json'},opts.headers||{});if(sessionToken)headers['X-Session-Token']=sessionToken;return fetch(url,Object.assign({},opts,{headers}));}
function showLicenseScreen(){document.getElementById('app').innerHTML=`<div class="lic-wrap animate-in"><div class="lic-card"><div class="lic-logo">Scriptblox</div><div class="lic-sub">Generator &nbsp;&middot;&nbsp; v2.6</div><span class="lic-label">LICENSE KEY</span><input class="lic-input" id="licInput" type="text" placeholder="KUNI-XXXX-XXXX-XXXX" autocomplete="off" spellcheck="false"><button class="lic-btn" id="licBtn" onclick="doLogin()">Verify License</button><div class="lic-err" id="licErr"></div><div class="price-section"><div class="price-title">PRICING</div><div class="price-grid"><div class="price-card"><div class="price-name">BASIC</div><div class="price-limit">100 accounts</div><div class="price-amt">$59.99</div><div class="price-php">&asymp; &#8369;3,389</div><div class="price-dur">1 Per Avail</div></div><div class="price-card featured"><div class="price-name">PRO</div><div class="price-limit">500 accounts</div><div class="price-amt">$269.99</div><div class="price-php">&asymp; &#8369;14,124</div><div class="price-dur">1 Per Avail</div></div><div class="price-card"><div class="price-name">UNLIMITED</div><div class="price-limit">no limit</div><div class="price-amt">$599.99</div><div class="price-php">&asymp; &#8369;22,599</div><div class="price-dur">30 days</div></div></div><div class="discord-section"><div class="discord-label">join our server to purchase</div><a class="discord-btn" href="https://discord.gg/GFGqB5Uj" target="_blank"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z"/></svg>Join <strong>Kuni Server</strong></a></div></div><div class="lic-footer"><span><span class="lic-dot" id="connDot"></span>kuni tool</span><span>v2.6</span></div></div></div>`;document.getElementById('licInput').addEventListener('keydown',e=>{if(e.key==='Enter')doLogin();});}
async function doLogin(){const key=document.getElementById('licInput').value.trim(),err=document.getElementById('licErr'),btn=document.getElementById('licBtn'),dot=document.getElementById('connDot');if(!key){err.style.color='#f5c842';err.textContent='please enter a license key';return;}btn.classList.add('loading');btn.textContent='Verifying...';err.style.color='#4a6070';err.textContent='connecting...';dot&&dot.classList.add('active');try{const[hwid,fp]=await Promise.all([getDeviceFingerprint(),getExtraFp()]),ls_token=getLsToken(),res=await fetch('/verify-key',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key,hwid,ls_token,fp})}),data=await res.json();if(data.valid){sessionToken=data.session_token;localStorage.setItem('license',key);localStorage.setItem('session_token',sessionToken);if(data.ls_token)localStorage.setItem('_kuni_lst',data.ls_token);licenseInfo=data;err.style.color='#00e87a';err.textContent='license valid - loading...';setTimeout(showMainApp,600);}else{btn.classList.remove('loading');btn.textContent='Verify License';err.style.color='#ff3b5c';err.textContent=ERR_MAP[data.error]||'invalid license - contact Kuni';dot&&dot.classList.remove('active');}}catch{btn.classList.remove('loading');btn.textContent='Verify License';err.style.color='#ff3b5c';err.textContent='server error';dot&&dot.classList.remove('active');}}
function showMainApp(){const planLabel=licenseInfo?licenseInfo.plan:'-',isUnlimited=licenseInfo&&licenseInfo.limit>=9999;document.getElementById('app').innerHTML=`<div class="app animate-in"><div class="hdr"><div><div class="hdr-logo">KUNI</div><div class="hdr-sub">AUTO SB GEN</div></div><div class="hdr-right"><span class="plan-badge" id="planBadge">${planLabel}</span><div class="hdr-ver">v2.6</div></div></div><div class="status-bar idle" id="statusBar"><span class="dot"></span><span class="status-text" id="statusText">idle - ready</span></div><div class="warn-banner" id="webhookWarn"><span class="warn-icon">!</span><div><strong>Discord webhook required.</strong> Set your webhook below before running the generator.</div></div><div class="stats"><div class="stat s-created"><span class="stat-val" id="s-created">0</span><span class="stat-lbl">CREATED</span></div><div class="stat s-active"><span class="stat-val" id="s-active">0</span><span class="stat-lbl">ACTIVE</span></div><div class="stat s-failed"><span class="stat-val" id="s-failed">0</span><span class="stat-lbl">FAILED</span></div><div class="stat s-target"><span class="stat-val" id="s-target">0</span><span class="stat-lbl">TARGET</span></div></div>${!isUnlimited?`<div class="limit-bar-wrap"><div class="limit-bar-top"><span>ACCOUNT USAGE</span><span id="limitText">-</span></div><div class="limit-bar-track"><div class="limit-bar-fill" id="limitBar" style="width:0%"></div></div></div>`:''}<div class="limit-banner" id="limitBanner">Account limit reached - contact Kuni to upgrade your plan.</div><div class="config-card"><div class="config-card-hdr" onclick="toggleConfig()"><span>CONFIG</span><span id="cfgToggle" style="font-size:9px;letter-spacing:2px">HIDE</span></div><div class="config-card-body" id="cfgBody"><div class="config-row"><div class="config-field"><span class="config-label">COUNT</span><input class="config-input" type="number" id="count" value="10" min="1" max="9999"></div><div class="config-field"><span class="config-label">CONCURRENT</span><input class="config-input" type="number" id="concurrent" value="10" min="1" max="50"></div></div><div class="panel-wrap"><div class="panel-label" onclick="togglePanel('proxyPanel')"><span>PROXIES</span><span class="badge" id="proxyBadge">loading...</span></div><div class="panel-inner" id="proxyPanel"><textarea class="panel-textarea" id="proxyTA" placeholder="paste proxies here - clears after save&#10;host:port, host:port:user:pass, http://user:pass@host:port"></textarea><div class="panel-actions"><button class="panel-btn" onclick="saveProxies()">SAVE</button><span class="panel-status" id="proxySt"></span></div></div></div><div class="webhook-wrap" id="webhookWrap"><div class="webhook-row"><span class="webhook-lbl">WEBHOOK <span id="webhookReqMark" style="color:var(--red)">*</span></span><input class="webhook-input" id="webhookInput" type="text" placeholder="https://discord.com/api/webhooks/..."></div><div class="webhook-req-note">Required - test ping sent on save to verify the URL works.</div><div class="panel-actions"><button class="panel-btn" onclick="saveWebhook()">SAVE &amp; TEST</button><span class="panel-status" id="webhookSt"></span></div></div></div></div><button class="run-btn idle" id="mainBtn" onclick="toggle()">Run Generator</button><div class="tutorial-card"><div class="tutorial-hdr" onclick="toggleTutorial()"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polygon points="10 8 16 12 10 16 10 8" fill="currentColor" stroke="none"/></svg><span style="letter-spacing:2px;font-size:10px">HOW TO USE YOUR ACCOUNTS</span><span id="tutToggle" style="margin-left:auto;font-size:9px;letter-spacing:2px;color:var(--muted)">SHOW</span></div><div class="tutorial-body" id="tutBody"><div class="video-container"><iframe src="https://www.youtube.com/embed/35KDnej1hlI" class="video-el" frameborder="0" allowfullscreen allow="autoplay"></iframe></div></div></div><div class="log-wrap"><div class="log-header"><span>LOG OUTPUT</span><button class="log-clear" onclick="clearLog()">clear</button></div><div class="log-box" id="logBox"></div></div><div class="footer"><span>kuni tool</span><a href="#" onclick="doLogout();return false;">logout</a></div></div>`;loadSavedConfig();updateLimitBar();initSocket();}
...
function toggleTutorial(){tutOpen=!tutOpen;document.getElementById('tutBody').classList.toggle('open',tutOpen);document.getElementById('tutToggle').textContent=tutOpen?'HIDE':'SHOW';}
function updateWebhookUI(hasWebhook){webhookSet=hasWebhook;const wrap=document.getElementById('webhookWrap'),warn=document.getElementById('webhookWarn'),mark=document.getElementById('webhookReqMark');if(!wrap||!warn)return;if(hasWebhook){wrap.classList.remove('required');warn.classList.remove('show');if(mark)mark.style.display='none';}else{wrap.classList.add('required');warn.classList.add('show');if(mark)mark.style.display='inline';}}
async function loadSavedConfig(){try{const r=await authFetch('/get-proxies'),d=await r.json();if(d.ok){const badge=document.getElementById('proxyBadge');badge.textContent=d.count>0?d.count+' loaded'+(d.source==='file'?' (proxies.txt)':''):'none';badge.className='badge'+(d.count>0?' ok':'');}}catch{}try{const r=await authFetch('/get-webhook'),d=await r.json();if(d.ok){updateWebhookUI(d.has_webhook);const st=document.getElementById('webhookSt');if(st){if(d.has_webhook){st.style.color='var(--green)';st.textContent='webhook set';}else{st.style.color='var(--red)';st.textContent='not set - REQUIRED';}}}}catch{}}
async function saveProxies(){const text=document.getElementById('proxyTA').value,st=document.getElementById('proxySt');st.style.color='var(--muted)';st.textContent='saving...';try{const r=await authFetch('/set-proxies',{method:'POST',body:JSON.stringify({proxies:text})}),d=await r.json();if(d.ok){st.style.color='var(--green)';st.textContent=d.count+' proxies saved';document.getElementById('proxyTA').value='';const b=document.getElementById('proxyBadge');b.textContent=d.count+' loaded';b.className='badge'+(d.count>0?' ok':'');setTimeout(()=>togglePanel('proxyPanel'),800);}else{st.style.color='var(--red)';st.textContent=d.error||'error';}}catch{st.style.color='var(--red)';st.textContent='request failed';}}
async function saveWebhook(){const wh=document.getElementById('webhookInput').value.trim(),st=document.getElementById('webhookSt');st.style.color='var(--muted)';st.textContent=wh?'testing webhook...':'clearing...';try{const r=await authFetch('/set-webhook',{method:'POST',body:JSON.stringify({webhook:wh})}),d=await r.json();if(d.ok){if(wh){st.style.color='var(--green)';st.textContent=d.tested?'verified - test sent':'saved';updateWebhookUI(true);}else{st.style.color='var(--gold)';st.textContent='cleared';updateWebhookUI(false);}}else{st.style.color='var(--red)';st.textContent=d.error==='invalid_webhook'?'invalid discord url':d.error==='webhook_unreachable'?'test failed - check url':d.error==='not_authenticated'?'session expired - refresh':(d.error||'error');}}catch{st.style.color='var(--red)';st.textContent='request failed';}}
function updateLimitBar(){if(!licenseInfo||licenseInfo.limit>=9999)return;const used=licenseInfo.limit-(licenseInfo.accounts_left||0),limit=licenseInfo.limit,pct=Math.min(100,Math.round(used/limit*100)),fill=document.getElementById('limitBar'),txt=document.getElementById('limitText');if(fill){fill.style.width=pct+'%';fill.className='limit-bar-fill'+(pct>=90?' danger':pct>=70?' warn':'');}if(txt)txt.textContent=used+' / '+limit+' ('+(100-pct)+'% left)';}
function showLimitReached(used,limit,reason){const banner=document.getElementById('limitBanner');if(banner){banner.classList.add('show');banner.innerHTML=(reason==='revoked'?'License revoked':reason==='expired'?'License expired':'Account limit reached - contact Kuni to upgrade.');}const btn=document.getElementById('mainBtn');if(btn){btn.className='run-btn disabled-btn';btn.textContent='Unavailable';}setStatus('limit',(reason||'limit reached')+' - '+used+'/'+limit);}
...
async function doLogout(){try{if(sessionToken)await fetch('/logout',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({session_token:sessionToken})});}catch{}localStorage.removeItem('license');localStorage.removeItem('session_token');sessionToken=null;location.reload();}
function initSocket(){socket=io({auth:{token:sessionToken}});socket.on('auth_failed',()=>{localStorage.removeItem('license');localStorage.removeItem('session_token');sessionToken=null;alert('Session expired - please log in again.');location.reload();});window.toggle=function(){if(running){socket.emit('stop');return;}if(!webhookSet){setStatus('stopped','webhook required - set it first');const warn=document.getElementById('webhookWarn');if(warn){warn.classList.add('show');warn.scrollIntoView({behavior:'smooth',block:'center'});}cfgOpen=true;document.getElementById('cfgBody').style.display='flex';document.getElementById('cfgToggle').textContent='HIDE';const input=document.getElementById('webhookInput');if(input)input.focus();return;}const count=parseInt(document.getElementById('count').value)||10,concurrent=parseInt(document.getElementById('concurrent').value)||10;socket.emit('start',{count,concurrent});};socket.on('started',d=>{running=true;const btn=document.getElementById('mainBtn');btn.className='run-btn stop';btn.textContent='Stop';setStatus('running','running - '+d.count+' accounts');});socket.on('start_blocked',d=>{running=false;setStatus('stopped',d.message||'blocked');if(d.reason==='webhook_required'){updateWebhookUI(false);const warn=document.getElementById('webhookWarn');if(warn)warn.scrollIntoView({behavior:'smooth',block:'center'});}});socket.on('stopped',()=>{running=false;const btn=document.getElementById('mainBtn');btn.className='run-btn idle';btn.textContent='Run Generator';setStatus('stopped','stopped');});socket.on('done',d=>{running=false;const btn=document.getElementById('mainBtn');btn.className='run-btn idle';btn.textContent='Run Generator';setStatus('done','done - '+d.created+'/'+d.total+' created');});socket.on('stats',d=>{document.getElementById('s-created').textContent=d.created;document.getElementById('s-active').textContent=d.active;document.getElementById('s-failed').textContent=d.failed;document.getElementById('s-target').textContent=d.target;if(licenseInfo&&licenseInfo.limit<9999){const used=(licenseInfo.used||0)+d.created,pct=Math.min(100,Math.round(used/licenseInfo.limit*100)),fill=document.getElementById('limitBar'),txt=document.getElementById('limitText');if(fill){fill.style.width=pct+'%';fill.className='limit-bar-fill'+(pct>=90?' danger':pct>=70?' warn':'');}if(txt)txt.textContent=used+' / '+licenseInfo.limit+' ('+Math.max(0,100-pct)+'% left)';}});socket.on('log',d=>{const box=document.getElementById('logBox');if(!box)return;const line=document.createElement('div');line.className='log-line '+(d.tag||'dim');const msg=d.msg||'',m=msg.match(/^\[(\d{2}:\d{2}:\d{2})\]\s*(.*)/s);if(m)line.innerHTML='<span class="log-ts">'+m[1]+'</span><span class="log-msg">'+m[2]+'</span>';else line.innerHTML='<span class="log-msg">'+msg+'</span>';box.appendChild(line);box.scrollTop=box.scrollHeight;});socket.on('limit_reached',d=>{running=false;showLimitReached(d.used||0,d.limit||0,d.reason);});}
const savedKey=localStorage.getItem('license'),savedToken=localStorage.getItem('session_token');
if(savedKey&&savedToken){(async()=>{const[hwid,fp]=await Promise.all([getDeviceFingerprint(),getExtraFp()]),ls_token=getLsToken();try{const r=await fetch('/verify-key',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key:savedKey,hwid,ls_token,fp})}),d=await r.json();if(d.valid){sessionToken=d.session_token;localStorage.setItem('session_token',sessionToken);licenseInfo=d;showMainApp();}else{localStorage.removeItem('license');localStorage.removeItem('session_token');showLicenseScreen();}}catch{showLicenseScreen();}})();}else{showLicenseScreen();}
</script>
</body>
</html>
"""
